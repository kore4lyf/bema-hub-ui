...
// Google Identity Services types (required for new OAuth clients)
declare global {
  interface Window {
    google: {
      accounts: {
...
export function GoogleLoginButton() {
  ...
  useEffect(() => {
    // Initialize Google Identity Services (modern, secure approach)
    ...
    if (window.google && window.google.accounts && window.google.accounts.id) {
      ...
      window.google.accounts.id.prompt((notification: any) => {
        if (notification.isNotDisplayed() || notification.isSkippedMoment()) {
          console.log('One Tap not displayed - user needs to click button again or use different method');
        }
      });
    } else {
      toast.error('Google login is not ready. Please try again.');
    }
  };
  ...
  const handleCredentialResponse = async (response: any) => {
    try {
      ...
      console.log('Sending social login data:', socialData);
  ...
  return (
  <Button variant="outline" type="button" onClick={handleLoginClick} disabled={isLoading} className="w-full">
...
export function FacebookLoginButton() {
  ...
  useEffect(() => {
    // Load Facebook SDK
    if (typeof window !== 'undefined' && !(window as any).fbLoaded) {
